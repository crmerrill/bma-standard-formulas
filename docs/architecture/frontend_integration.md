...
- Solver tab:
  ...
  - guardrailed waterfall target primitives:
...
- Structuring deals and solver:
  ...
  - `GET /deals/{deal_id}/runs/{run_id}/progress` includes optional `diagnostic_artifacts` once solve artifacts are persisted

The solver catalog now returns typed enum families for:

- objective types
- constraint operators
- waterfall target primitives
- template family target packs (`PRIME_JUMBO`, `NON_QM_QRM`)
...
Solver diagnostics generated by orchestration and visualized in Analysis include:

- CE ladder table (`*_solver_ce_ladder`)
- cumulative-loss-multiple coverage matrix (`*_solver_loss_multiple_coverage`)
- OC/IC trigger breach timeline (`*_solver_trigger_breach_timeline`)
- step-down gate status (`*_solver_stepdown_gate_status`)
