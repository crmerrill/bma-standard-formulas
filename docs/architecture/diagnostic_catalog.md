...
| IR_VALIDATION_ERROR | error | {field_loc} | Pydantic validation failed: {msg} | backend | Correct the invalid field value reported in the diagnostic path. | src/bma_cfengine_app/orchestrator/deals/validation_service.py:26 |
| RULE_FRAGMENTATION_CONSOLIDATABLE | warning | deal.waterfall_rules[start_index..end_index] | Rules {start_index} through {end_index} can be consolidated into one multi-target rule. | both | canonicalize_consolidate_rule_run | canonicalization_validators.py:23 |
| STALE_QUICKFIX | warning | deal.waterfall_rules | QuickFix could not be applied to range [{start_index}..{end_index}]: {reason}. | both | (none) | canonicalization_validators.py:82 |
| INTERLEAVED_RULES_FACTORABLE | info | deal.waterfall_rules[{indices}] | Rules at {indices} share (rule_type, source, payment_style) but are interleaved with a source mutation; manual review recommended. | both | Manual review only; no automatic fix is offered. | canonicalization_validators.py:106 |
