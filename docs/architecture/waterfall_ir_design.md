...
|---|---|---|---|---|
| 1 | **Discount Option** — pre-waterfall reclassification of a % of newly-generated principal as finance charges | MEDIUM | Chase, Capital One, Citi all reference; Chase shows it currently inactive (yield factor = 0) | **Implemented** (Phase 7): `DealDefinition.discount_factor_pct` (0–100%). Runtime applies before rule dispatch. Legacy `deal_knobs["discount_factor"]` still accepted during migration. |
...
- Discount Option via `DealDefinition.discount_factor_pct` (promoted from deal_knobs in SR7)
...
it serves multiple purposes. Several reserved key families have
...
- All `BondDef` fields except `schedule_derivation` (always machine-written)
...
### Hybrid fields: `deal_knobs`

`deal_knobs` is written by **both** humans and the backend:
- Human authors use it for solver scratch-pad entries and expression globals.
- The solver backend writes proposed values (`deal_knobs.class_a_pctbal` etc.)
  during optimization runs.
- Index rates (`deal_knobs["SOFR_rate"]`) may come from either a human override
  or a future rate-curve integration.

`deal_knobs` is therefore *not* purely human-authored. Backend-written entries
should not be treated as user preferences; they may be overwritten on the next
solve run.
...
- **`BondDef.schedule_contract`** — can be either:
  - *Hand-authored*: explicit `{period, target_balance}` points entered by
    the analyst (custom-vector mode, prospectus-extracted tables).
  - *Machine-generated*: output of `build_psa_schedule_overlay()`. Machine-
    generated schedules are identified by the presence of `schedule_derivation`.
  Both forms survive round-trips via `block.data` stashing and
  `extractScheduleOverlayFromIr`; only PSA-derived contracts (with
  `schedule_derivation`) participate in stale detection.
- **`BondDef.schedule_derivation`** — machine-written provenance block;
  records the PSA inputs (speeds, pool balance/WAC/term, support stack) used to
  derive the schedule. Never hand-authored; absent for custom-vector contracts.
- **`BondDef.schedule_tolerance_bps`** — can be hand-set; defaults to
  25 bps when the PSA tool derives the schedule.
...
class BondDef(BaseModel):
    ...
    # Round 3 (May 2026): TrancheType (13) + TrancheBehavior (4) → TrancheKind (8).
    kind: TrancheKind                     # CASH_PAY | PAC | TAC | IO | PO | Z | RESIDUAL | PSEUDO
    is_bond: bool = True                  # False for residual/pseudo sinks
    is_pseudo: bool = False               # True for fee-pay sinks

    # Coupon — Phase 5 (May 2026): each rate field accepts scalar OR RateOrSchedule
    # (list[RateScheduleEntry]) for step-up/lockout coupons.
    coupon_type: CouponType = FIXED       # FIXED | FLOATING | INVERSE_FLOATING | ZERO
    coupon: RateOrSchedule | None         # Annual % (5.5 = 5.5%); inverse floater strike rate
    margin: RateOrSchedule | None         # Floater spread over index (can be step-up schedule)
    index_name: str | None                # "SOFR" | "TERM_SOFR_1M" | etc.
    cap: RateOrSchedule | None            # Rate cap (secondary ceiling for inverse floater too)
    floor: RateOrSchedule | None          # Rate floor
    inverse_multiplier: float | None      # Leverage for inverse floater: coupon = strike - mult×index + margin
    ...
    # The speed fields are DESIGN-TIME inputs for PSA derivation; the runtime
    # ONLY reads schedule_contract. TAC is the degenerate band (low == high).
    schedule_model_type: PrepayModelType | None  # PSA | CPR | ABS | CUSTOM_VECTOR
    schedule_speed_low: float | None
    schedule_speed_high: float | None     # == schedule_speed_low for TAC
    schedule_custom_vector: str | None
    schedule_priority_tier: int | None
    schedule_depends_on: str | None

    # schedule_contract: the authoritative schedule used by the runtime.
    # Can be hand-authored (explicit period targets) OR machine-generated by
    # build_psa_schedule_overlay(). When machine-generated, schedule_derivation
    # records the PSA inputs so stale detection works correctly.
    schedule_contract: list[dict]         # [{period, target_balance}]
    schedule_tolerance_bps: float | None  # default 25 bps for PAC/TAC
    schedule_derivation: dict | None      # machine provenance (PSA inputs + method)

    # Tranche relationships — Round 3 H (May 2026)
    ...
    # COUPON_INVERSE_OF, COUPON_LEVERAGE_OF, MACR_EXCHANGE are schema-only
    # (validation warnings only; no runtime cashflow effect).
    relations: list[TrancheRelation]
    ...
    # Phase 6 (May 2026): Credit-card NLA tracking
    nla_starting_balance: float | None    # Nominal Liquidation Amount starting value
    required_subordination_pct: float | None  # % of balance required as subordination
    seniority: int | None                 # 1 = most senior; used for available subordination calc

    # Multi-group
    group_id: str | None                  # "GROUP_1", "GROUP_2"

    pay_mode: PayMode = CASH_PAY          # CASH_PAY | PIK
```

**Reusability principle:** one `BondDef` covers all bond types.
`kind` (TrancheKind) expresses the bond's structural identity; `relations`
expresses its ties to other bonds; `coupon_type` + `coupon`/`margin`/
`cap`/`floor` express its interest mechanics; `schedule_contract` (with
optional machine provenance in `schedule_derivation`) expresses the
PAC/TAC planned-balance contract.
...
class AccountDef(BaseModel):
    ...
    # Phase 7 (May 2026): Funding Account accumulation period schedule.
    # Per-period minimum balance targets for PFA/IFA accounts. Sticky:
    # the effective minimum for period N is the highest entry with period ≤ N.
    # Duplicate periods are rejected by the validator.
    minimum_schedule: list[AccountMinimumScheduleEntry] | None
```
...
class TriggerNode(BaseModel):
    ...
    calculation_ref: str | None           # Pointer to a CalculationNode for dynamic thresholds or deal_knobs numeric key
    comparison_ref: str | None            # Name of a CalculationNode for a dynamic threshold value
    cure_periods: int | None              # How many clean periods to clear the trigger

    # Phase 9 (May 2026): rolling-window triggers for CC master trust pay-out events
    window_periods: int | None            # Average metric over last N periods before comparing threshold
    comparison: str = ">"                 # ">" | ">=" | "<" | "<=" — polarity of threshold comparison
                                          # Use "<" for excess-spread pay-out triggers (fire when below floor)
```
